...
mlcc.fit <- MPCV.BIC(comp, numb.clusters = 1:3, numb.runs = 20, max.dim = 3, greedy=F, estimate.dimensions=T, numb.cores=1)
summary(mlcc.fit)
...
```{r}
mlcc.fit2 <- MPCV.reps(X=comp, numb.clusters = 2, numb.runs = 20, max.dim = 4, numb.cores=1)
...
```{r}
mlcc.fit3 <- MPCV.reps(comp, numb.clusters = 2, numb.runs = 0, max.dim = 2, initial.segmentations=list(benchmarkClustering), numb.cores=1)
