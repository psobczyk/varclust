...
pca.new.BIC <- function(X, k){
  ...
  if(any(lambda < 0)){
    #eigen jest taka średnia jak się okazuje -> powodowało to dla małych danych zwracanie NaNa z cluster.pca.BIC
...
cluster.pca.BIC <- function(X, segmentation, dims, numb.clusters, max.dim, flat.prior = FALSE){
  ...
  for(k in 1:numb.clusters){
    ...
    if(dim(Xk)[2] > dim1){
      ...
      formula[k] <- - Inf
      warning("dziwny pomysl ja bym to przemyslal jeszcze")
    }
  }
  #apriori
...
choose.cluster.BIC <- function(variable, pcas, numberClusters){
  ...
  for(i in 1:numberClusters){
    ...
    if (sigma.hat < 1e-15){
      #do mierzenia czasu
      #przy inicjalizacji warning wyświetla się za każdym razem, pomysł -> parametr verbose
...
calculate.distance.kmeanspp <- function(variable, pcas, numberClusters){
  ...
  min(dists)
} #reszty BIC nie trzeba uwzględniać bo jest taka sama dla każdego klastra i zmiennej (patrz wyprowadznie na likelihood)
