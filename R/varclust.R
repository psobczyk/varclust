...
#' probabilistic model, which assumes that each clusters lies in a low 
...
#' orthogonal factors and similarity between a given variable and a cluster 
#' center depends on residuals from a linear model fit. Based on the Bayesian 
#' Information Criterion (BIC), sums of squares of residuals are appropriately 
#' scaled, which allows to avoid an over-excessive attraction by clusters with 
#' larger dimensions. To reduce the chance that the local minimum of BIC is 
#' obtained instead of the global one, for every fixed number of clusters in a 
#' given range K-means algorithm is run large number of times, with different 
...
#' clusters and their dimensions are computed using BIC. If the number
#' of clusters is known one might use function \code{\link{mlcc.reps}}, which 
#' takes number of clusters as a parameter. For \code{\link{mlcc.reps}} one 
#' might specify as well some initial segmentation for k-means algorithm. This 
#' can be useful if user has some a priori knowledge about clustering.
#' 
...
#' segmentation. The other measure can is implemented as 
#' \code{\link{integration}} function.
#' 
#' @docType package
#' @name varclust
